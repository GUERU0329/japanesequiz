...
function ScenarioQuizPage() {
  const { quizId } = useParams();
  const navigate = useNavigate();
  const quizItem = quizMeta.find((item) => item.id === quizId);
  ...
  const totalSteps = quizData?.steps.length ?? 0;
  ...
  const fireCorrectConfetti = () => {
    ...
    const interval = window.setInterval(() => {
      ...
      confetti({ particleCount: 36, angle: 60, spread: 55, origin: { x: 0, y: 0.72 } });
      confetti({ particleCount: 36, angle: 120, spread: 55, origin: { x: 1, y: 0.72 } });
    }, 250);
  };
  ...
  const replayCurrentAudio = () => {
    ...
    if (phase === "scene" && stepData?.duration) {
      ...
      progressTimerRef.current = window.setInterval(() => {
        ...
        setSceneProgress(Math.min(elapsed / stepData.duration, 1));
      }, 100);
  ...
  const togglePause = () => {
    if (phase !== "scene" && phase !== "question" && phase !== "reaction") return;
    ...
    if (!audioRef.current && phase === "scene" && stepData?.duration && isPlaying && !isPaused) {
      const elapsed = Date.now() - sceneStartRef.current;
    ...
    if (!audioRef.current && phase === "scene" && stepData?.duration && isPaused) {
      sceneStartRef.current = Date.now() - (stepData.duration - remainingTimeRef.current);
  ...
  useEffect(() => {
    if (phase !== "scene" || !stepData) return;
  ...
  useEffect(() => {
    if (phase !== "question" || !quizData) return;
  ...
  }, [phase, isMuted]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const progressLabel = useMemo(() => {
    if (phase === "scene") return `Scene ${currentStep + 1} / ${totalSteps}`;
    if (phase === "question") return "Question";
    if (phase === "reaction") return "Reaction";
    if (phase === "explanation") return "Explanation";
    return "準備完了";
  ...
  const renderSceneBar = () => (
    <div className="video-bar">
      {quizData.steps.map((_, index) => {
        const isDone =
          phase === "question" || phase === "reaction" || phase === "explanation" || index < currentStep;
        const isActive = phase === "scene" && index === currentStep;

        return (
          <div key={index} className="video-segment">
            <div
              className={`video-segment-fill ${isDone ? "done" : ""} ${isActive ? "active" : ""}`}
              style={{
                width: isDone ? "100%" : isActive ? `${sceneProgress * 100}%` : "0%",
              }}
            />
          </div>
        );
        ...
        <div className="quiz-shell">
          <div className="question-screen">
            <div className="question-card">
              <h2>この場面別クイズが見つかりません</h2>
              <Link to="/" className="sub-button" style={{ textDecoration: "none" }}>
                ホームに戻る
              </Link>
            </div>
          </div>
      ...
      <div className="quiz-shell">
        <div className="top-bar">
          <div className="title-block">
            ...
            <h1>{phase === "intro" ? quizData.title : quizData.title}</h1>
          </div>
          <div className="status-chip">{progressLabel}</div>
        </div>

        {(phase === "scene" || phase === "question" || phase === "reaction" || phase === "explanation") &&
          renderSceneBar()}
        ...
        {phase === "countdown" && (
          <div className="countdown-screen">
            <div className="countdown-ring">{countdown > 0 ? countdown : "START"}</div>
          </div>
        )}
              ...
              <div className="question-top">
                ...
                <div className="question-top-right">
                  <div className="question-timer-inline">
                    ...
                    <span className="question-timer-inline-seconds">{questionTimeLeft}s</span>
                  </div>
                  {isPlaying && <div className="now-playing question-playing">Now Playing</div>}
                </div>
              </div>
          ...
          <div className="scene-screen">
            <div className="scene-card">
              <div className="image-frame">
                ...
                <div className="speaker-tag">{selectedChoice.reaction.speaker}</div>
                {renderOverlayControls()}
              </div>
              ...
              <div className="scene-actions reaction-actions">
                ...
                <div className="reaction-buttons">
                  ...
                  <button className="sub-button" onClick={() => setPhase("explanation")}>
        ...
        {phase === "explanation" && (
          <div className="question-screen">
            <div className="question-card explanation-card">
              ...
              <div className="scene-actions reaction-actions">
                <button className="sub-button" onClick={() => setPhase("reaction")} disabled={!selectedChoice}>
                  リアクションに
                  <ruby>戻<rt>もど</rt></ruby>る
                </button>
                <div className="reaction-buttons">
...
export default ScenarioQuizPage;
